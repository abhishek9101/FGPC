...
import songsData from '../assets/course.json'

function CourseSong({ navigation }) {
  ...
  if (loading) {
    return (
      <View style={[styles.container, { backgroundColor: colors.background, justifyContent: 'center', alignItems: 'center' }]}>
        <Text style={{color:"lightBlue",marginBottom:20,fontSize:25, fontWeight:"bold"}}>Loading...</Text>
    ...
    <ScrollView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      {filteredSongs.map((song, index) => (
        <View key={song.songNumber}>
          <TouchableOpacity 
            onPress={() => navigation.navigate('CourseDetails', { songNumber: song.songNumber })}
          >
            <View style={[styles.songItem, { borderColor: colors.border }]}>
              <Text style={[styles.songNumber, { color: colors.text }]}>Chorus Number: {song.songNumber}</Text>
